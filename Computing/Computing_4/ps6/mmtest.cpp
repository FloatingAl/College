...
BOOST_AUTO_TEST_CASE(order0) {
  ...
  // length of input in constructor
...
BOOST_AUTO_TEST_CASE(order2) {
  ...
  // kgram is wrong length
  BOOST_REQUIRE_THROW(mm.freq("", 'g'), std::runtime_error);
  // kgram is wrong length
  BOOST_REQUIRE_THROW(mm.freq("x", 'g'), std::runtime_error);
  // kgram is wrong length
  BOOST_REQUIRE_THROW(mm.freq("xxx", 'g'), std::runtime_error);
